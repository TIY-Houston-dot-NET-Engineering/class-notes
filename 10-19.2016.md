...
public class Program {
    // taking an multi-digit int and returning them as an array of single digit integers
    public static int[] digitize(int num){
<<<<<<< HEAD
        return num // multi-digit int
          .ToString() // convert to string
          .ToCharArray() // convert to char[]
          .Select(c => Int32.Parse(c+"")) // convert to IEnumerable<int>
...
>>>>>>> 2a7df17f14af6121b209690b9a2b6bf15e68af9d
    }
...
public class Program {
    public static bool isIsogram(string s){
      char[] list = s // convert char[] to string
                    .ToLower() // convert to lowercase
                    .ToCharArray(); // convert tochar[]

      return list // char[]
        .Where(c => Array.IndexOf(list, c) != Array.LastIndexOf(list, c)) // IEnumerable<char> and checking if there are multiple indexes of any letter
        .Count() // int
          == 0; // bool
    }
...
}               //after completion... all returns = true
```
...
Array.IndexOf() // static properties and methods do not operate on an instance

new char[]{}.ToString() // instance prop/methods are used in a class and can be called using this
```
...
<<<<<<< HEAD
                    ...
                    //this is the deserialized info from the Google API page
                    //using this portion to identify the portion of namespace Weather2 that will be called from the GoogleAPIModel
                    //we deleted the portions of the site that we will not need
                    //class GoogleAPIModel contains List<Result> results that holds the lat and lng of the location
