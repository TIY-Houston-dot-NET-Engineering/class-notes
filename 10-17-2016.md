...
class MainClass {
    ...
    // with aggregates you are required to pass in the accumulator before passing in the lambda
    // non-variadic (i.e. sum4(new int[]{1,2,3,4,5}) )
    static int sum4(IEnumerable<int> nums) =>       // this func is utilizing a foreach loop
        nums.Aggregate(0, (sum, num) => sum+num);

    // variadic (i.e. sum5(1,2,3,4,5,6...) )
    static int sum5(params int[] nums) {
        return nums.Aggregate(0, (sum, num) => sum+num);        //aggregates take a lambda and an initial value and returns a single value
    }
...
<<<<<<< HEAD
    ...
    // to a new array
=======
    // Concat expects a list of arguments 
>>>>>>> 2a7df17f14af6121b209690b9a2b6bf15e68af9d
    ...
    public void Add2(IEnumerable<int> nums){
        foreach(int n in nums){
            Add(n); // Add method expects an individual value
        }
        ...
        //     if(v%2 == 1) // is odd
        //         return acc.Concat(new int[]{a}); // Concat method expects a collection of values
        //     return acc;
        // }, new List<int>());
...
```
// .Select statement is a foreach loop, taking a collection and turning it into a new collection.
